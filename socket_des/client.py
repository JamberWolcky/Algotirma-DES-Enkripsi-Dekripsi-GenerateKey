...
# Fungsi DES dan fungsi tambahan yang diperlukan
...
def client_program():
    ...
    key = "abcd1234"  # Kunci enkripsi yang sama dengan server

    while True:
        # Ambil input dari client, enkripsi pesan, lalu kirim ke server
        client_message = input("Client, masukkan pesan untuk server (atau 'exit' untuk keluar): ")
        encrypted_message = des_encrypt(client_message, key)
        encrypted_message_bits = ''.join(map(str, encrypted_message))
        
        # Menampilkan hasil enkripsi di sisi client sebelum dikirim ke server
        print(f"Hasil enkripsi pesan client: {encrypted_message_bits}")
        
        # Kirim pesan terenkripsi ke server
        client_socket.send(encrypted_message_bits.encode())
        print("Pesan terenkripsi berhasil dikirim ke server.")
        ...
        decrypted_text = des_decrypt(cipher_bits_response, key)
        
        # Tampilkan pesan terenkripsi dan hasil dekripsinya
        print(f"Pesan terenkripsi yang diterima dari server: {encrypted_response}")
        print(f"Pesan didekripsi dari server: {decrypted_text}")

        # Akhiri jika server mengirim 'exit'
        if decrypted_text.lower() == 'exit' or client_message.lower() == 'exit':
